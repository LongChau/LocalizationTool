...
public class CSVSerializer
{
    ...
    //@LONG
    static public T[] CustomDeserialize<T>(string text)
    {
        return (T[])CustomCreateArray(typeof(T), ParseCSV(text));
    }

    static public T[] CustomDeserialize<T>(List<string[]> rows)
    {
        return (T[])CustomCreateArray(typeof(T), rows);
    }

    static private object CustomCreateArray(Type type, List<string[]> rows)
    {
        Array array_value = Array.CreateInstance(type, rows.Count - 1);
        Dictionary<string, int> table = new Dictionary<string, int>();
        ...
        for (int i = 1; i < rows.Count; i++)
        {
            object rowdata = CustomCreate(rows[i], table, type);
            array_value.SetValue(rowdata, i - 1);
        }
        return array_value;
    }

    static object CustomCreate(string[] cols, Dictionary<string, int> table, Type type)
    {
        object v = Activator.CreateInstance(type);
